...
int main(int argc, char *argv[])
{
  ...
  int dims[2];
  int newid, nbrbottom, nbrtop, nbrleft, nbrright, nbrtopleft, nbrtopright, nbrbottomleft, nbrbottomright;
  ...
  int memsizes[ndims];
  int mycoords[ndims];
  int topleftcoords[ndims];
  int toprightcoords[ndims];
  int bottomleftcoords[ndims];
  int bottomrightcoords[ndims];
  ...
  MPI_Cart_coords(comm2d, newid, ndims, mycoords);
 
  nbrtopleft = nbrtopright = nbrbottomleft = nbrbottomright = MPI_PROC_NULL;


  if (newid%2 != 0 && newid != 1) {
    topleftcoords[0] = mycoords[0]-1;
    topleftcoords[1] = mycoords[1]-1;
    MPI_Cart_rank(comm2d, topleftcoords, &nbrtopleft);
  }

  if (newid%2 == 0 && newid != 0) {
    toprightcoords[0] = mycoords[0]-1;
    toprightcoords[1] = mycoords[1]+1;
    MPI_Cart_rank(comm2d, toprightcoords, &nbrtopright);
  }

  if (newid%2 != 0 && newid != numprocs-1) {
    bottomleftcoords[0] = mycoords[0]+1;
    bottomleftcoords[1] = mycoords[1]-1;
    MPI_Cart_rank(comm2d, bottomleftcoords, &nbrbottomleft);
  }

  if (newid%2 == 0 && newid != numprocs-2) {
    bottomrightcoords[0] = mycoords[0]+1;
    bottomrightcoords[1] = mycoords[1]+1;
    MPI_Cart_rank(comm2d, bottomrightcoords, &nbrbottomright);
  }

  printf("myid %d, newid %d, top %d, bottom %d, left %d, right %d, topleft: %d, topright: %d, bottomleft: %d, bottomright: %d \n", 
      myid, newid, nbrtop, nbrbottom, nbrleft, nbrright, nbrtopleft, nbrtopright, nbrbottomleft, nbrbottomright);
  ...
  for (iter = 1; iter <= NMAXITER; iter++) {
    ...
    for(i = 1; i < myrows + 1; i++) {
      for(j = 1; j < mycols + 1; j++) {
	mynew[i][j] = (1.0/20.0) * (myold[i-1][j-1] + 4 * myold[i-1][j] 
	            + myold[i-1][j+1] + 4 * myold[i][j-1] + 4 
	            * myold[i][j+1] + myold[i+1][j-1] + 4 
	            * myold[i+1][j] + myold[i+1][j+1]) - (h*h/40) 
	            * (myf[i-1][j] + myf[i][j-1] + 8 * myf[i][j] 
	            + myf[i][j+1] + myf[i+1][j]);
    ...
    if (diff_global < CRIT_PAR) { /* Todos terminam.*/
      ...
      MPI_File_open(comm2d, "array2d_c.bin", MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);
    ...
    MPI_Sendrecv(&mynew[1][mycols], 1, coluna, nbrright, 3,
        ...
        comm2d, MPI_STATUS_IGNORE);    
    MPI_Sendrecv(&mynew[1][1], 1, MPI_DOUBLE, nbrtopleft, 4,
        &mynew[myrows+1][mycols+1], 1, MPI_DOUBLE, nbrbottomright, 4, 
        comm2d, MPI_STATUS_IGNORE);
    MPI_Sendrecv(&mynew[myrows][mycols], 1, MPI_DOUBLE, nbrbottomright, 5,
        &mynew[0][0], 1, MPI_DOUBLE, nbrtopleft, 5, 
        comm2d, MPI_STATUS_IGNORE);
    MPI_Sendrecv(&mynew[1][mycols], 1, MPI_DOUBLE, nbrtopright, 6,
        &mynew[myrows+1][0], 1, MPI_DOUBLE, nbrbottomleft, 6, 
        comm2d, MPI_STATUS_IGNORE);
    MPI_Sendrecv(&mynew[myrows][1], 1, MPI_DOUBLE, nbrbottomleft, 7,
        &mynew[0][mycols+1], 1, MPI_DOUBLE, nbrtopright, 7, 
        comm2d, MPI_STATUS_IGNORE);
